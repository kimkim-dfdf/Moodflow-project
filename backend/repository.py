...
from models import db, User, Task, EmotionHistory, Emotion, Music, BookTag, Book, MusicFavorite
...
# ==============================================
# Music Favorite Operations
# ==============================================

def get_user_music_favorites(user_id):
    """
    Get all favorite music for a user.
    Returns list of music dictionaries.
    """
    favorites = MusicFavorite.query.filter_by(user_id=user_id).all()
    
    result = []
    for fav in favorites:
        music = db.session.get(Music, fav.music_id)
        if music:
            music_dict = music.to_dict()
            music_dict['favorite_id'] = fav.id
            result.append(music_dict)
    
    return result


def get_user_favorite_music_ids(user_id):
    """
    Get list of music IDs that user has favorited.
    Returns set of music IDs for quick lookup.
    """
    favorites = MusicFavorite.query.filter_by(user_id=user_id).all()
    
    result = set()
    for fav in favorites:
        result.add(fav.music_id)
...
def add_music_favorite(user_id, music_id):
    ...
    Returns the created favorite or None if already exists.
    """
    existing = MusicFavorite.query.filter_by(
        user_id=user_id, 
        music_id=music_id
    ).first()
...
def remove_music_favorite(user_id, music_id):
    ...
    """
    favorite = MusicFavorite.query.filter_by(
        user_id=user_id,
        music_id=music_id
    ).first()
...
def get_similar_music(user_id, limit=6):
    """
    Get music recommendations based on user's favorites.
    Algorithm: Find music with same genre or emotion as favorited music.
    Uses simple scoring: +2 for same genre, +1 for same emotion.
    """
    favorites = get_user_music_favorites(user_id)
    ...
    for fav in favorites:
        favorite_ids.add(fav['id'])
        favorite_genres.append(fav['genre'])
        favorite_emotions.append(fav['emotion'])
    ...
    for music in all_music:
        ...
        for emotion in favorite_emotions:
            if music.emotion == emotion:
                score = score + 1
        
        if score > 0:
            music_dict = music.to_dict()
            music_dict['similarity_score'] = score
            scored_music.append(music_dict)
    
    for i in range(len(scored_music)):
        for j in range(len(scored_music) - 1):
            if scored_music[j]['similarity_score'] < scored_music[j + 1]['similarity_score']:
                temp = scored_music[j]
                scored_music[j] = scored_music[j + 1]
                scored_music[j + 1] = temp
    
    if limit and limit < len(scored_music):
        return scored_music[:limit]
